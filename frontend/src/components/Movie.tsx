import { Component, createEffect, createMemo, For, Match, Show, splitProps, Switch } from "solid-js";
...
const Movie: Component = () => {
  const h = createEventSignal<Record<string, MessageEvent<string>>>(source, "message", {
    passive: true,
  });

  const parsedData = createMemo(() => {
    const result = ToolControllerOutput.safeParse(JSON.parse(h()?.data || "null"));
    if (result.success) {
      return result.data
    }
    return null
