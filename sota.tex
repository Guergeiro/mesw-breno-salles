...
\author{
  \IEEEauthorblockN{1\textsuperscript{st} Breno Salles}
  \IEEEauthorblockA{
    \textit{Faculty of Engineering} \\
    \textit{University of Porto}\\
    Lisboa, Portugal \\
    business@brenosalles.com
  }
}
\date{January 2023}
...
following a specific methodology to identify, analyse, and interpret all
...
and analyse the most relevant publications for our purposes. Through this
...
often rigid and not easily adjustable.
...
Freitas, Ferreira and Cunha's \cite{freitas2021refactoring} tool,
MicroRefact \footnote{\url{https://github.com/FranciscoFreitas45/MicroRefact}},
utilizes a Java source code to extract structural information by relying on the
Abstract Syntax Tree \cite{aho2007compilers}. This information is used to
generate a list of candidate microservices. They then leverage Spring Boot
decorators, particularly those utilizing the Java Persistence API (JPA), to
infer the entities of the database and their relationships. This process then
results in the output of working Java code for each identified microservice.
...
independent services. In fact, when programming language was not a determining
factor, OpenAPI was commonly used as the standard for distinguishing
microservices from monolithic systems.

The tool proposed by Al-Debagy and Martinek \cite{al2019new} utilizes the
...
\cite{sun2022expert} uses a similar approach to identify microservices
within a monolithic application. The first step is calculating the similarity
...
model as input \cite{staffa2021pangaea} or utilizing the history of changes to
understand in addition to common artifacts like classes and methods
\cite{santos2021microservice}.
...
The tool proposed by Al-Debagy and Martinek \cite{al2019new} uses OpenAPI as
...
\cite{krishna2021transforming} \cite{kalia2021mono2micro}
\cite{kalia2020mono2micro} \cite{nakazawa2018visualization}. This additional
information can be very beneficial for architects and developers, as it makes
it easier for them to understand the connections and dependencies between
microservices, and make informed decisions about how to proceed with
...
employ this method. One of them is the work of Freitas, Ferreira and Cunha
\cite{freitas2021refactoring}, and another is Mono2Micro
\cite{krishna2021transforming} \cite{kalia2021mono2micro}
\cite{kalia2020mono2micro} that in an update explained in a YouTube video
...
input \cite{al2021microservice} \cite{bucchiarone2020model}
\cite{assunccao2022analysis} \cite{zhao2021applying} \cite{nitin2022cargo}
\cite{kalia2021mono2micro} \cite{kalia2020mono2micro}
\cite{agarwal2021monolith} \cite{pigazzini2019tool}
\cite{krishna2021transforming}. This may be attributed to the language's strict
syntax rules, which facilitate the examination of source code during the
inspection process. Additionally, some of them utilized the Spring Boot
framework in conjunction with Java \cite{wei2020feature}
\cite{nunes2019monolith} \cite{brito2021identification}
\cite{santos2021microservice} \cite{freitas2021refactoring}
\cite{pigazzini2019tool}, which further enforces structure through the
utilization of decorators. In contrast, those who employed programming
languages other than Java, such as Python, utilized corresponding frameworks
like Django, to compensate for the language's more lenient syntax constraints
\cite{matias2020determining} \cite{asseldonk2021monolith}.
